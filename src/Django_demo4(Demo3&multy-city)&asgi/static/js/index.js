...
function init() {
    ...
    var weatherStr = weather[0].str;
    var result = JSON.parse(weatherStr);

    for(var i=0;i<7;i++)
    {

        Tmax[i]=result.weather[i].tmax;
        Tmin[i]=result.weather[i].tmin;
...
function getCity() {
    ...
    if (isNumber(string)||issign(string)||string=="") {
        ...
        /*$.ajax({
            type:"GET",
            url:"127.0.0.1:8000/city?city=shanghai",
            success:function(result){
                alert(result[0].tmax);
                for(var i=0;i<7;i++){

                Tmax[i]=result[i].tmax;
                Tmin[i]=result[i].tmin;
                }
            }
            error:function(){
                alert("error!");
...
//});
        ...
        xmlhttp.open("get","city?city="+string,true);
